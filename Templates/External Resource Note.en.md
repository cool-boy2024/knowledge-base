---
type: external-resource   # or 'tool' for software/products you use
created: <% tp.date.now("YYYY-MM-DD") %>
url:

# Edge fields (typed relationships — consumed by KG pipeline)
up: []              # Parent MOC or index
area: []            # Area of focus this serves
concept: []         # Concepts discussed or realized
author: []          # Wikilink to author note if one exists
related: []         # Lateral connections

tags:
  - "#status/draft"

# Bilingual linkage
lang: en
translation_of: "[[External Resource Note]]"

# Curator fields (uncomment if ontology fit is uncertain)
# curator_status: pending
# curator_suggested_type:
# curator_observations:
#   - ""
---

# <% tp.file.title %>

<p align="right">
  <strong>🌐 语言 / Language:</strong>
  <a href="External%20Resource%20Note.md"><img src="https://img.shields.io/badge/%E4%B8%AD%E6%96%87-switch-lightgrey?style=for-the-badge" alt="中文"></a>
  <a href="External%20Resource%20Note.en.md"><img src="https://img.shields.io/badge/English-current-blue?style=for-the-badge" alt="English (current)"></a>
</p>

> One-sentence summary: what this is and why it matters.

**Author**:
**Source**:
**Date**:

---

## What It Does

[2-3 paragraphs: what this is, how it works, what problem it solves. For tools, focus on capabilities. For articles/posts, capture the key argument or insight.]

---

## Connections

[Wikilinks to related vault notes — concepts, other tools, literature. Let connections emerge from the content rather than forcing research relevance.]

---

## Source

- URL:
