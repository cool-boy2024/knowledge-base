...
> Reference documentation from Anthropic. Source: https://code.claude.com/docs/en/memory
> This file helps Claude Code understand its own memory system when modifying vault configuration.
...
Keep it to facts Claude should hold in every session: build commands, conventions, project layout, "always do X" rules. If an entry is a multi-step procedure or only matters for one part of the codebase, move it to a [skill](/docs/en/skills) or a [path-scoped rule](#organize-rules-with-clauderules) instead. The [extension overview](/docs/en/features-overview#build-your-setup-over-time) covers when to use each mechanism.
